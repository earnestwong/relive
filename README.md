...
本 Fork 在原版基础上做了以下优化，使其**无需依赖墨水屏硬件**，即可利用旧 iPad、电脑、手机等设备实现每日 AI 挑选照片的展示。

### 1. 网页幻灯片展示（slideshow.html）

新增 `slideshow.html`，可通过 Docker 部署为独立服务，在任何支持浏览器的设备上播放每日 AI 挑选的展示照片。

- **全屏自适应**：照片根据设备屏幕自动缩放，保持原始比例不裁切
- **自动轮播**：支持自动播放、手动切换，底部进度条显示播放进度
- **多设备兼容**：iPad、电脑、手机均可使用，适合利用旧设备作为电子相框
- **独立部署**：通过 Docker 容器运行，读取 Relive 后端生成的每日展示批次图片

### 2. 离线 GPS 省份映射修正

原版代码中 GeoNames FIPS admin1 代码与中国省份的映射存在 5 处错误，已全部修正：
...
修正后，离线 GPS 解析能正确识别中国省份和城市，支持直辖市显示到区级（如"上海市杨浦区"）。

### 3. 全彩图展示照片横竖版自适应

原版代码固定将所有展示图片裁切为 480×800 竖版，已优化为根据原图方向自动适配：

| 原图方向 | 全彩图尺寸 | 照片区 | 信息区 |
|---------|-----------|--------|--------|
| 横版 | 1024×768 | 668px (87%) | 100px (13%) |
| 竖版 | 768×1024 | 924px (90.2%) | 100px (9.8%) |
